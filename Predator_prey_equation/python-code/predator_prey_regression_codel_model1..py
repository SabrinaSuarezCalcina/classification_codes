# -*- coding: utf-8 -*-
"""
Created on Tue Jun  9 18:20:43 2019
...
for j in range(10,50):
    fname_features = '/File_CSV/TDA_Beta_features_m_%d.csv' %(j)    
    fname_pars = '/File_CSV/Beta_Parameter_values_m_%d.csv' %(j)
    print('item', j)
    ...
    for veces in range(0,30):
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)
        
        modelknn = regress_knn(10, X_train, X_test, y_train, y_test)
        matrix1[veces,0] = modelknn[0] # R^2 result
        matrix1[veces,1] = modelknn[1] # rmse result
        ...
        matrix2[veces,0] = modelsvr[0] # R^2 result
        matrix2[veces,1] = modelsvr[1] # rmse result
        matrix2[veces,2] = par_C       # C parameter
        matrix2[veces,3] = par_nu      # Nu paramter
        print('veces', veces)
        
    final1 = np.array(matrix1)
    np.savetxt("/Results-files/R2_rmse/knn_measures_m%d.txt"%(j), final1, fmt="%.14f", delimiter=",")

    final2 = np.array(matrix2)
    np.savetxt("/Result-files/R2_rmse/svr_measures_m%d.txt"%(j), final2, fmt="%.14f", delimiter=",")
print('fim')
